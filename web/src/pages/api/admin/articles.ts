...
export const POST: APIRoute = async ({ request }) => {
  ...
  if (action === 'save') {
    ...
    try {
      ...
      return json({ ok: false, error: 'Could not save the article. Please try again.' }, 500);
    }
  }
  ...
  if (action === 'delete') {
    ...
    try {
      ...
      return json({ ok: false, error: 'Could not delete the article. Please try again.' }, 500);
    }
  }
